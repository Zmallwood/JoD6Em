...
namespace jod
{
  ...
  {
    if (message == "CanvasSize")
    {
      auto canvSize = GetCanvasSize();
      int msg[3];
      msg[0] = MessageCodes::k_canvasSize;
      msg[1] = canvSize.w;
      msg[2] = canvSize.h;
      if (auto result =
              emscripten_websocket_send_binary(m_webSocketEvent->socket, msg, 3 * sizeof(int)))
  ...
  namespace
  {
                   ...
                   void *userData)
    {
      ...
      if (auto result =
              emscripten_websocket_send_utf8_text(websocketEvent->socket, "Initialize connection"))
        std::cout << "Failed to send init message to server: " << result << std::endl;

      _<WebSocketClient>().SendMessage("CanvasSize");
