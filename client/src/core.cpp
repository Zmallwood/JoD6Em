...
namespace jod {
    namespace {
        ...
        EM_BOOL touch_start_callback(int,
                                     EmscriptenTouchEvent const *,
                                     void *);
        EM_BOOL touch_end_callback(int,
                                   EmscriptenTouchEvent const *,
                                   void *);
    ...
    client_engine::run() const {
        ...
        emscripten_set_main_loop(game_loop_function,
                                 0,
    ...
    input_manager::input_manager(){
        ...
        glfwSetKeyCallback(_<graphics>().m_window,
                           key_callback);
        // Set callback for mouse events.
        glfwSetMouseButtonCallback(_<graphics>().m_window,
                                   mouse_button_callback);
        // Set callback for text typing events.
        glfwSetCharCallback(_<graphics>().m_window,
                            character_callback);
        // Set callback for touch start event.
        emscripten_set_touchstart_callback("#canvas",
                                           nullptr,
                                           true,
                                           touch_start_callback);
        // Set callback for touch end event.
        emscripten_set_touchend_callback("#canvas",
                                         nullptr,
                                         true,
                                         touch_end_callback);
    }
    ...
    render_instructions_manager::execute_instructions(){
        ...
        for (auto &instr : m_imageDrawInstructions)
            _<image_renderer>().draw_image(instr.rid,
                                           instr.imageNameHash,
                                           instr.dest);
    }
    namespace {
