...
// Current mosue position in the browser canvas.
    PointF mousePosition;
// Size of canvas in browser.
    Size canvasSize;
// Handles mouse input and provides state.
    std::unique_ptr<MouseInput> mouseInput;
// Manages game scenes for the user s instance.
    std::unique_ptr<SceneManager> sceneManager;
// Player instance for the user.
    std::unique_ptr<Player> player;
// Custom cursor which replaces default system cursor.
    std::unique_ptr<Cursor> cursor;
// Server side FPS counter.
    std::unique_ptr<ServerFPSCounter> serverFPSCounter;
...
// Update the current scenes logic.
    m_pImpl->sceneManager->UpdateCurrScene(userID);
...
// Render the current scene.
    m_pImpl->sceneManager->RenderCurrScene(userID);
