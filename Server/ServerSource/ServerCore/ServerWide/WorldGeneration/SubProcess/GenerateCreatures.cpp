...
// World area to generate creatures in.
    WorldArea* worldArea;
// Name of creature to generate.
    std::string creatureName;
// Number of groups of the creature type to generate.
    int numGroups;
// Number of creatures in one generated group.
    int numCreaturesInGroup;
// Area in the world area where the groups will be spawned.
    Box area;
// Level of creatures that are generated, determines how strong they area.
    int creatureLevel;
};
...
// Dont spawn creatures outside the world area.
            if (!args.worldArea->IsValidCoord({x,y})) continue;
// Dont spawn on top of another already existing creature.
            if (args.worldArea->GetTile(x, y)->GetCreature()) continue;
// Dont spawn creatures in the water.
            if (args.worldArea->GetTile(x, y)->GetGround() ==
                Hash("GroundWater"))
                continue;
