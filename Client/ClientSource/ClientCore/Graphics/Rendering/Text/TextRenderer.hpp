...
// Stored font objects of different sizes.
    std::map<FontSizes, std::unique_ptr<const Font>> m_fonts;
// Unique names of images created to render text.
    std::map<RID, std::string> m_uniqueNameIDs;
// Map where key is imageID and value is RID used with ImageRenderer.
    std::map<RID, RID> m_ridsGLResources;
// File system path to fonts location.
    const std::string k_relFontsPath{"Resources/Fonts/"};
// Color of outline of all rendered text.
